...
async def get_available_orders_for_delivery(
    ...
    try:
        ...
        orders = await db.find_many(
            "orders",
            {
                "order_status": {"$in": ["confirmed", "pending"]},
                "delivery_partner": {"$exists": False}  # Not assigned to any delivery partner yet
            },
            sort=[("created_at", -1)]
        )
...
async def accept_delivery_order(
    ...
    try:
        ...
        if order.get("order_status") not in ["confirmed", "pending"]:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
        ...
        await db.update_one(
            ...
            {"_id": order_object_id},
            {
                "$set": {
                    ...
                    "assigned_at": db.get_current_datetime()
                }
            }
        )
...
@router.post("/delivery/{order_id}/mark-delivered")
